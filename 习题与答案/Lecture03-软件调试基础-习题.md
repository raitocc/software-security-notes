...
2. 下列哪一项通常用于记录 PE 文件依赖的外部 DLL 和函数信息？B
...
3. Optional Header 中，和程序入口点位置关系最密切的字段是：A
...
6. 在 IDA 中，用于查看二进制文件中字符串并跳转到引用位置的窗口是：A
...
7. OllyDbg 中，单步步入和单步步过对应的快捷键分别是：A
...
8. 下列关于 `MessageBoxA` 的说法，和课件注入示例最贴近的是：A
...
9. PE 文件中可以包含图标、菜单、位图等资源信息。T

10. 用户态调试器中看到的程序地址通常是虚拟地址。T

11. IDA 的 Graph View 会把函数拆成基本块并显示控制流关系。T

12. 在 PE 注入示例中，写入注入代码后，还需要修改入口点或跳转逻辑，让程序控制流到达这段代码。T
...
22. 在 IDA 文本视图中，若某条跳转以虚线箭头显示，较可能表示：A
...
23. OllyDbg 中使用“查找所有引用的字符串”定位 `wrong password`，主要是为了：A
...
24. IAT 中填入的导入函数地址，和 DLL 实际加载位置有关。T

25. 文件中的 section 较紧凑，而内存中的 section 往往要按页或较大粒度对齐。T

26. ASLR 改变的是地址布局，程序逻辑本身仍由原来的指令决定。T

27. IDA 的反编译伪代码有助于理解逻辑，但分析关键分支时仍应结合汇编。T
...
35. `.text` 节加载到内存后的起始 VA 是多少？411000
...
38. 原入口点 VA 是多少？1003E21
39. 修改 PE 入口点时应填写的新 RVA 是多少？4ABA
40. `jmp 0x01003E21` 的作用是什么？回去
41. 为什么 `call MessageBoxA` 能被解析到正确函数？导入表IAT
